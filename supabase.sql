...
-- V10.3: hero slider + payment method (idempotent / aman dijalankan pada setup baru)
...
create policy "hero public read" on public.hero_slides for select using (active = true or public.is_admin());
drop policy if exists "hero admin insert" on public.hero_slides;
create policy "hero admin insert" on public.hero_slides for insert to authenticated with check (public.is_admin());
drop policy if exists "hero admin update" on public.hero_slides;
create policy "hero admin update" on public.hero_slides for update to authenticated using (public.is_admin()) with check (public.is_admin());
drop policy if exists "hero admin delete" on public.hero_slides;
create policy "hero admin delete" on public.hero_slides for delete to authenticated using (public.is_admin());
alter table public.orders add column if not exists payment_method text default 'BCA';
alter table public.orders drop constraint if exists orders_payment_method_check;
alter table public.orders add constraint orders_payment_method_check check (payment_method in ('BCA','Mandiri','BRI','GoPay'));
