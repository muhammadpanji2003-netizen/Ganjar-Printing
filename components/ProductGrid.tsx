...
export default function ProductGrid({limit}:{limit?:number}){
  ...
  return <><div className="catalogFilters">{cats.map(c=><button key={c} className={cat===c?'active':''} onClick={()=>setCat(c)}>{c}</button>)}</div><div className="productGrid">{shown.map(p=><article className="productCard" key={p.id}><Link href={`/produk/${p.slug}`} className="productPhoto">{p.image?<img src={p.image} alt={p.name}/>:<div className="productPlaceholder"><ImageIcon/><span>Ganjar Printing</span></div>}{p.featured&&<span className="featuredBadge">Pilihan</span>}</Link><div className="productBody"><span className="productCategory">{p.category}</span><Link href={`/produk/${p.slug}`}><h3>{p.name}</h3></Link><p>{p.description||'Layanan printing berkualitas dengan proses pemesanan yang mudah.'}</p><div className="productBottom"><div><small>Mulai dari</small><b>Rp{p.price.toLocaleString('id-ID')} <em>{p.unit}</em></b></div><div className="productQuick"><a href={`https://wa.me/${wa}?text=${encodeURIComponent(`Halo Ganjar Printing, saya ingin bertanya tentang ${p.name}.`)}`} target="_blank" rel="noreferrer"><MessageCircle/></a><Link href={`/produk/${p.slug}`}><ArrowUpRight/></Link></div></div></div></article>)}</div></>;
}
